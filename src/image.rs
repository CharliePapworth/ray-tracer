...
impl Image {
    ...
    pub fn over(&self, under: &Image) -> Image {
        ...
        image.pixels = self
            .pixels
            .iter()
            .zip(under.pixels.iter())
            .map(|(over_pixel, under_pixel)| over_pixel.over(under_pixel))
            .collect();
        image
    }
...
impl<'a> Add for &'a RaytracedImage {
    ...
    fn add(self, other: &'a RaytracedImage) -> RaytracedImage {
        ...
        output.image.pixels = self
            .image
            .pixels
            .iter()
            .zip(other.image.pixels.iter())
            .map(|(a, b)| Pixel::new(a.color + b.color, 1.0))
            .collect();
        output
    }
}
