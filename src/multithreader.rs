...
impl Multithreader {
    ...
    pub fn send_update<F>(&self, update_function: F) where F: Fn() {
 ...
 pub fn run_thread(thread_data: Arc<RwLock<ThreadData>>, film: Arc<Mutex<Film>>, film_tiles_in_progress: Arc<ArrayQueue<FilmTile>>, film_tiles_finished: Arc<ArrayQueue<FilmTile>>, multithreader_to_thread_rx: Receiver<Instructions>, function: Arc<Box<dyn Fn(ThreadData, &mut FilmTile, Receiver<Instructions>) -> Option<Instructions> + Send + Sync + 'static>>) {
