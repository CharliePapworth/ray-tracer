...
impl Sphere{
    ...
    pub fn wrap_horizon(&self, cam: &Camera) -> Option<Vec<Line3>>{
        ...
        for i in 0..NUMBER_OF_LINES {
            ...
            let line_start = cam.origin + origin_horizon_center + horizon_basis_vector_a * f32::cos(old_angle) 
                                                                      + horizon_basis_vector_b * f32::sin(old_angle);

            let line_end = cam.origin + origin_horizon_center + horizon_basis_vector_a * f32::cos(new_angle) 
...
impl Hit for Sphere{
    ...
    fn bounding_box(&self) -> Option<Aabb> {
        let output_box = Aabb::new(self.center - Vector3::<f32>::new(self.radius, self.radius, self.radius),
